...
- `app/services/chat_graph/nodes.py`: 히스토리 없음 분기 복원 및 JSON 출력 혼합 대응 파싱 추가.
- `app/services/chat_graph/nodes.py`: LangChain 메시지 타입에서 현재 질문/히스토리 분리 처리 추가.
- `app/services/chat_graph/summaries.py`: 로그 출력에서 텍스트 잘림 제거.
- `app/api/routes.py`: `/api/ask` 문서에 히스토리/현재 질문 분리 설명 추가.
- `README.md` 및 폴더별 `README.md`: 모델/공통 모듈/프롬프트 평가 변경사항 문서 최신화.
- `docs/prompt/`: 프롬프트 원본 분리, SemVer 가이드/초기 버전 추가.
- `app/services/shared/prompts.py`: 프롬프트 파일 로더 추가 및 캐시 적용.
- `app/services/chat_graph/helpers.py`, `app/services/prompt_eval/runner.py`: 프롬프트 파일 로딩 및 버전 로그 출력 추가.
- `app/utils/config.py`, `.env.example`: 프롬프트 버전/루트 설정 추가.
- `app/utils/config.py`: 프롬프트 버전 ENV 오버라이드 제거.
- `app/ui/streamlit_app.py`: 평가자별 상세 표에 평가 모델명을 표기.
