...
- 프롬프트 평가: 모범답변(reference) 옵션 추가로 평가 루브릭 분기, 결과 카드/텍스트/다운로드 UX 개선.
- UI 그래프: Graphviz 기반으로 채팅/프롬프트 평가 흐름을 탭별 토글로 표시.
- Swagger: `/api/prompt-eval`에 reference_answer 옵션 안내 추가.
- 가이드: 설정/환경변수 중앙관리 규칙을 `docs/guides/config-guidelines.md`로 분리 정리.
- 평가 요약 UI를 DataFrame 없이 테이블+리스트로 개편(순위/평균점수/평가자별 점수·근거 분리).
