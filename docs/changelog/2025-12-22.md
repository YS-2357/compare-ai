...
- `README.md` 및 폴더별 `README.md`: 최신 모델/공통 모듈/프롬프트 평가 변경사항 반영.
- `docs/prompt/`: 프롬프트 원본 분리 및 SemVer 가이드 추가.
- `app/services/shared/prompts.py`: 프롬프트 로더 추가, 서비스에서 파일 기반 프롬프트 사용.
- `app/utils/config.py`, `.env.example`: 프롬프트 버전/루트 설정 추가.
- `app/utils/config.py`: 프롬프트 버전을 config 고정으로 변경(ENV 오버라이드 제거).
