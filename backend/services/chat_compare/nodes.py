...
from backend.services.shared.errors import build_status_from_error, build_status_from_response
from .providers import NODE_CONFIG, default_active_nodes
from .prompts import Answer, build_chat_prompt, build_chat_prompt_input, message_to_text, render_chat_history

from backend.services.shared.llm_registry import (
    ChatAnthropic,
    ChatCohere,
...
async def call_model_common(
    ...
    *,
    ...
    logger.debug(
        "call_model_common:시작 label=%s models=%s",
        label,
    ...
    try:
        ...
        updated_msgs, summaries = await maybe_summarize_history(llm, state, label, [("assistant", msg_payload)])
...
async def maybe_summarize_history(llm: Any, state: GraphState, label: str, new_messages: list) -> tuple[list, dict[str, str]]:
    """
    모델별 히스토리를 갱신하고 필요 시 요약한다.
    - 최근 메시지가 일정 수 이상 쌓이면 최근 MAX_CONTEXT_MESSAGES 기준으로 요약
    - 요약은 model_summaries[label]에 1개만 유지하고, 히스토리는 최근 메시지만 보존
    """

    logger.debug("maybe_summarize_history:시작 label=%s", label)
    ...
    should_summarize = len(recent_messages) >= 3
    if should_summarize:
        text_lines = []
        ...
        logger.info(
            "history/summarize label=%s msgs_before=%d msgs_after=%d summary_len=%d preview=%s",
            label,
            len(existing),
            len(recent_messages),
            len(summary),
...
def init_question(state: GraphState) -> GraphState:
    ...
    logger.debug("init_question:종료 active_models=%d", len(active_models))
    return result
