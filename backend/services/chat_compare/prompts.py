...
from backend.services.shared import load_prompt
...
def build_chat_prompt_input(state: Any, label: str) -> str:
    ...
    if history_text.strip():
        ...
        prompt = (
            "[Current Question]\n"
            f"{current_question}\n\n"
            "There is no prior conversation history. Answer clearly and only in Korean."
        )
        mode = "no_history"
    logger.info(
        "prompt/built label=%s mode=%s chars=%d preview=%s",
        label,
        mode,
        len(prompt),
        preview_text(prompt),
